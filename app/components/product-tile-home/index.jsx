...
/**
 * The ProductTileHome is a simple visual representation of a
...
const ProductTileHome = (props) => {
    ...
    const {currency, id, price} = product
    const image = product.imageGroups[0].images[0]

    // ProductTileHome is used by two components, RecommendedProducts and ProductList.
    ...
    return (
        <Link
            ...
            {...styles.container}
            to={productUrlBuilder({id: id}, intl.local)}
            {...rest}
        >
            <Box {...styles.imageWrapper}>
